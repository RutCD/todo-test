defmodule TodoTest.AccountingTest do
  ...
  describe "prices" do
    ...
    test "create_price/1 with valid data creates a price" do
      valid_attrs = %{descriptio: "some descriptio", name: "some name", price: 120.5}

      assert {:ok, %Price{} = price} = Accounting.create_price(valid_attrs)
      assert price.descriptio == "some descriptio"
    ...
    test "update_price/2 with valid data updates the price" do
      price = price_fixture()
      update_attrs = %{descriptio: "some updated descriptio", name: "some updated name", price: 456.7}

      assert {:ok, %Price{} = price} = Accounting.update_price(price, update_attrs)
      assert price.descriptio == "some updated descriptio"
